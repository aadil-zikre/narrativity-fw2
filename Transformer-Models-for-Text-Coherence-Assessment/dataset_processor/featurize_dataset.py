...
dataset_options = {
        ...
        "airbnb" : {
            "tasks" : ['sentence-score-prediction']
        }
}
...
class AirbnbFeaturizer:
    def __init__(self, args):
        if args.task not in dataset_options['airbnb']['tasks']:
            print(dataset_options['airbnb'])
            raise Exception('handler for %s task is not defined.' % (args.task))

        args.logger.info('featurizing the airbnb corpus: %s for task: %s with model architecture: %s' % (args.sub_corpus, args.task, args.arch))
        self.args = args
    
    def get_task_specific_labels(self, data_input_map):
        if self.args.task == 'sentence-score-prediction':
            #regression task and it can be in range [0, 3]
            labels = int(data_input_map['narrativity_label'] == 'good')
            return labels
        else:
            raise Exception('%s task specific label processing is not defined' % self.args.task)
    ...
    def load_data(self, inference):
        ...
        if inference:
            ...
            raise NotImplementedError('Airbnb can only be used for Inference')
            # del dataset_map['test']

        airbnb_dataset_path = os.path.join(self.args.processed_dataset_path, 'AIRBNB')
        self.args.logger.debug('<<LOADING>> Airbnb dataset from directory: %s' % airbnb_dataset_path)
        self.args.logger.debug('working on %s dataset ' % 'test')
        file_name_ = "S4_sentence_len_matched_labelled_pairs_F1.jsonl"
        input_data = load_file(os.path.join(airbnb_dataset_path, file_name_))
        dataset_map['test'].extend(input_data)
        self.args.logger.debug('<Done>')
        self.args.logger.debug('--'*30)
        return dataset_map
    ...
    def featurize_dataset(self, inference=False):
            ...
            for set_type, data_list in dataset_map.items():
                for data_instance in data_list:
                    if self.args.arch in ['hierarchical', 'combined'] or mtl_with_hierarchical_model:
                        ...
                        data_instance['doc_a'] = data_instance['doc_a']
                    data_instance['label'] = self.get_task_specific_labels(data_instance)
        ...
        # featurizing the dataset
        data_list = dataset_map['test']
        save_file_path = os.path.join(save_dir, '%s.jsonl'%'test')
        featurize_dataset(tokenizer, fact_tokenizer, include_facts, self.args.max_fact_count, self.args.max_seq_len, 
                                        self.args.max_fact_seq_len, save_file_path, data_list, self.args.logger)
        self.args.logger.debug('--'*30)
