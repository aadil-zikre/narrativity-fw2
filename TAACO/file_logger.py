...
class Logger:
    ...
    def add_log_prefix(self, prefix):
        ...
        for handler in self.logger.handlers:
            if self.prefix:
                formatter = logging.Formatter(handler.formatter._fmt.replace("%(message)s", "") + f"{prefix} :: %(message)s")
    ...
    def remove_log_prefix(self):
        for handler in self.logger.handlers:
            if self.prefix:
                formatter = logging.Formatter(handler.formatter._fmt.replace(f"{self.prefix} :: %(message)s", "%(message)s"))
                handler.setFormatter(formatter)
        self.prefix = None
        return self.get_logger()
